...
rtl_fm  -f "$freq"M -M fm -s 12.5k  -r 12.5k - | ffmpeg -re -t $timeout -s 480x360 -f rawvideo -pix_fmt rgb24 -r 30 -i /dev/zero -f s16le -ac 1 -ar 12.5k -i pipe:0 -vf "$ffmpegprinttimefilter" -vcodec libx264 -preset veryfast -acodec libmp3lame -b:a 128k -ar 44100 -f flv rtmp://a.rtmp.youtube.com/live2/$rtmpkey -codec:a libmp3lame -b:a 128k "$sat""$fecha"_"$freq"MHZ.mp3 &
